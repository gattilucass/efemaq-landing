...
export default function Footer({ showWhatsapp = true }: { showWhatsapp?: boolean }) {
  ...
  const exploreLinks = (
    <ul className="space-y-3 pl-1 md:pl-0">
      {['Inicio', 'Administradores', 'Particulares', 'Blog', 'Nosotros'].map((item) => (
        <li key={item}>
          <Link
            href={item === 'Inicio' ? '/' : item === 'Administradores' ? '/para-administradores' : item === 'Particulares' ? '/para-particulares' : item === 'Blog' ? '/blog' : '#'}
